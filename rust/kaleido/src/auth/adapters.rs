use crate::auth::traits::{CooldownError, CooldownManager, CooldownType, EmailService};
...
impl SeaOrmCooldownBackend {
    ...
    fn action_for(t: crate::auth::traits::CooldownType) -> &'static str {
        match t {
            crate::auth::traits::CooldownType::Login => "login",
            crate::auth::traits::CooldownType::EmailResend => "email_resend",
            crate::auth::traits::CooldownType::EmailForgotPassword => "email_forgot_password",
        }
    }
}
...
impl CooldownBackend for SeaOrmCooldownBackend {
    async fn check_cooldown(
        &self,
        cooldown_type: crate::auth::traits::CooldownType,
        user_id: Option<i32>,
    ) -> Result<(), String> {
        use chrono::Utc;
        const COOLDOWN_SECS: i64 = 60;

        let rec = crate::auth::entities::cooldowns::Model::find_by(
            &self.db,
            "auth",
            user_id,
    ...
    async fn update_cooldown(
        ...
        crate::auth::entities::cooldowns::Model::upsert_last_run(
            &self.db,
            "auth",
            user_id,
            Self::action_for(cooldown_type),
    ...
    async fn record_failure(
        ...
        crate::auth::entities::cooldowns::Model::increment_attempts(
            &self.db,
            "auth",
            user_id,
            Self::action_for(cooldown_type),
    ...
    async fn reset_cooldown(
        ...
        crate::auth::entities::cooldowns::Model::reset_attempts(
            &self.db,
            "auth",
            user_id,
            Self::action_for(cooldown_type),
...
/// and config provider.
pub type DefaultAuthService<E, F> = crate::auth::services::AuthService<
    E,
...
where
    E: crate::auth::traits::EmailService,
    F: crate::auth::traits::ConfigProvider,
{
    crate::auth::services::AuthService::new(
        email,
...
#[async_trait]
impl crate::auth::traits::AuditLogger for SeaOrmAuditLogger {
    async fn log_event(
        &self,
        event_type: crate::auth::traits::AuthEventType,
        payload: crate::auth::traits::AuthEventPayload,
    ) -> Result<(), String> {
        let shared_event_type = match event_type {
            crate::auth::traits::AuthEventType::LoginSucceeded => {
                crate::auth::entities::auth_events::EventType::LoginSucceeded
            }
            ...
            crate::auth::traits::AuthEventType::TokenRefreshFailed => {
                crate::auth::entities::auth_events::EventType::TokenRefreshFailed
            }
            crate::auth::traits::AuthEventType::PasswordResetRequest => {
                crate::auth::entities::auth_events::EventType::PasswordResetRequest
            }
            crate::auth::traits::AuthEventType::PasswordReset => {
                crate::auth::entities::auth_events::EventType::PasswordReset
            }
        };

        let shared_payload = crate::auth::entities::auth_events::AuthEventPayload {
...
impl crate::auth::traits::MetricsRecorder for FnMetricsRecorder {
...
impl crate::auth::traits::ConfigProvider for StaticConfigProvider {
...
pub struct AuthTaskQueue {
    inner: crate::background_jobs::TaskQueue<crate::background_jobs::DurableStorage>,
}

impl AuthTaskQueue {
    pub fn new(db: sea_orm::DatabaseConnection) -> Self {
        let storage = crate::background_jobs::DurableStorage::new(db);
        Self {
            inner: crate::background_jobs::TaskQueue::new(storage),
        }
    }
...
impl EmailTaskDispatcher for AuthTaskQueue {
    async fn send_registration_email(&self, email: String, name: String, verification_url: String) {
        crate::auth::worker::tasks::enqueue_email_registration(
            &self.inner,
            email,
            name,
            verification_url,
    ...
    async fn send_password_reset_email(
        ...
        crate::auth::worker::tasks::enqueue_email_password_reset(
            &self.inner,
            email,
            name,
            reset_url,
...
impl crate::auth::traits::ConfigProvider for EnvConfigProvider {
    fn frontend_url(&self) -> &str {
        &self.frontend_url
    }
